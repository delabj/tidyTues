...
ggmap::ggmap(basemap) +
  geom_point(data =locations_date, aes(x=longitude, y=latitude, color  = season))+
  labs(
    title = "<b style='color:#721121'>Summer</b> & <b style='color:#AC78BA'>Winter</b> Migrations?",
    subtitle = "Locations of tagged caribou on {frame_time}",
    x=NULL,
    y=NULL,
    caption= "Data: Movebank \nViz: @delabjl"
    )+
  ...
  shadow_wake(wake_length = .25, alpha = .1)+
  theme_delabj()+
  scale_color_manual(values=c("#721121","#AC78BA"))+
  gridlines_off()+
    theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())+
